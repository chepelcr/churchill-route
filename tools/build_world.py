...
def stamp_service_worker():
    """Version the SW cache name with a content hash of the app shell so
    every changed deploy invalidates the previous offline cache. Run via
    the pipeline or standalone with --stamp-only (used by the deploy CI)."""
    import hashlib
    import re
    shell = ["index.html", "styles.css", "world.js", "engine.js", "ui.jsx",
             "tweaks-panel.jsx", "world-data.js", "manifest.webmanifest"]
    h = hashlib.sha1()
    for name in shell:
    ...
    new = re.sub(r'const CACHE = "churchill-[^"]*";',
                 f'const CACHE = "churchill-{ver}";', src, count=1)
    if new != src:
        with open(sw_path, "w", encoding="utf-8") as f:
            f.write(new)
        print(f"[sw] cache stamped: churchill-{ver}")
    else:
        print(f"[sw] cache unchanged: churchill-{ver}")
