...
//
// world-data.js is produced by tools/build_world.py from docs/map.osm and
// defines window.WORLD_DATA (real Puntarenas peninsula, corridor-unrolled:
// x = arclength along the Faro→Caldera spine, y = exaggerated perp offset).
// This file adapts that raw data into the runtime API the engine consumes:
// grid surface queries, silhouette functions, road arclength samplers and a
...
window.WORLD = (function () {
  ...
  // ----- Surface grid -------------------------------------------------------
  // Class ids: 0 water, 1 land, 2 beach, 3 road, 4 paseo, 5 bridge.
  ...
  // topY/botY arrays have one entry per 4px grid column (first/last non-water
  // row per column, in world px). Lerp between columns for smooth queries.
  ...
  // Each road: { cls, w, name?, ref?, pts (flat [x0,y0,...]), cum, len, aabb }.
  ...
  return {
    ...
    PALMS, MANGROVES, HILLS, ESTUARY, BRIDGE, PIER,
