...
// What-you're-about-to-play card for Recorrer y Arcade, so those modes get the
// same "here's the deal" beat Historia gets from StageBrief instead of dropping
// you straight into the puerto with no idea what the rules are.
...
export default function ModeBrief({ mode, brief = mode, onGo }) {
  ...
  const SLOTS = {
    // the MODE is the title — "MODO" alone read as the heading, with the mode
    // itself demoted to a subtitle under it
    heading: () => (
    ...
    body: () => <p style={{ opacity: 0.85, lineHeight: 1.5, fontSize: 13 }}>{t(`modebrief.${brief}.body`)}</p>,
    sky: () => (
    ...
    cycleNote: () => <p style={{ opacity: 0.7, fontSize: 12 }}>☀︎ {t("explore.cycle")}</p>,
    rules: () => (
    ...
    go: () => (
      <button className="btn gold" onClick={() => onGo({ weather: mode === "arcade" ? sky : undefined })}>
        {t("brief.go")}
      </button>
    ),
  };
  const ctx = { picksSky: mode === "arcade", turnsTheDay: brief === "explore" };
  const slot = (region) => <Slots screen={UI_SCREEN.MODEBRIEF} region={region} ctx={ctx} slots={SLOTS} />;
